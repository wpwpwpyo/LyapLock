...
from lyaplock import LyapLockHyperParams
...
def main(
    ...
    if 'LyapLock' in alg_name:
        kwargs = {
            ...
            'alpha': {l: alpha for l in hparams.layers},
    ...
    model_save_location = str(run_dir) + '/' + 'model/'
    cnt = 0
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE"]):
            ...
            if cnt == 0:
                edited_model, weights_copy, kwargs = apply_algo(
                    model,
                    tok,
                    [
                        {"case_id": record["case_id"], **record["requested_rewrite"]}
                ...
                edited_model, _, kwargs = apply_algo(
                    model,
                    tok,
                    [
                        {"case_id": record["case_id"], **record["requested_rewrite"]}
        ...
        if (args.downstream_eval_steps > 0 and cnt % args.downstream_eval_steps == 0) or cnt == len(ds) // num_edits:
            glue_results = {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit","MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE", "LyapLock"],
        default="ROME",
    ...
    parser.add_argument(
        ...
        type=float,
        default=60,
        help="Adjust the threshold D = alpha * D_base",
    )
