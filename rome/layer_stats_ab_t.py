...
from util.runningstats import CombinedStat, Mean, NormMean, SecondMoment, SecondMoment_AB_T, tally
...
def layer_stats_vk_t(
    ...
    def get_ds():
        ...
        raw_ds = Dataset.from_file('/public/wp/cache/.cache/huggingface/datasets/wikipedia/20200501.en/1.0.0/009f923d9b6dd00c00c8cdc7f408f2b47f45dd4f5fb7982a21f9448f4afbe475/wikipedia-train.arrow')
        raw_ds = {'train': raw_ds}
        # raw_ds = load_dataset(
        #     ds_name,
    ...
    file_extension = f"{model_name}/{ds_name}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}_vk_t.npz"
    filename = stats_dir / file_extension

    print(f"Computing VK_T locally....")
...
def layer_stats_kk_t(
    ...
    stats_dir = Path(stats_dir)
    file_extension = f"{model_name}/{ds_name}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}_kk_t.npz"
    filename = stats_dir / file_extension

    print(f"Computing KK_T locally....")
...
def layer_stats_vv_t(
    ...
    stats_dir = Path(stats_dir)
    file_extension = f"{model_name}/{ds_name}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}_vv_t.npz"
    filename = stats_dir / file_extension

    print(f"Computing VV_T locally....")
